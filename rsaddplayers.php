...
<!DOCTYPE html>
  <html>
    <head>
	    <title>Add Female Players</title>
    ...
    <body>
         ...
         if ($_SESSION['user_type'] == 'admin'||$_SESSION['user_type'] == 'Super admin'){
            //Super Admin has the ability for adding categories
              if(isset($_POST['add_female_player'])){
               ...
               $sql="INSERT INTO player(firstname,secondname,dateOfBirth,playerImage,startOfPlay,endOfPlay, description, gender) VALUES('$firstname','$secondname','$dateOfBirth','$imageContent','$startOfPlay','$endOfPlay','$description','Female')";
               if($conn->query($sql)) {
                       ?>
                        <center>
  ...
  <br><br>   
        ...
        if ($_SESSION['user_type'] == 'admin'||$_SESSION['user_type'] == 'Super admin'){
			?>
              ...
              <p id="profile-name" class="profile-name-card" style="font-size:18px;">Add female player
              </p>
...
<input class="btn btn-lg btn-primary btn-block btn-signin" type="submit" name="add_female_player" style="width:269px; height:40px;color:white;font-size:16px" value="Add female player">
