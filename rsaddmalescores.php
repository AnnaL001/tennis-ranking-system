...
<!DOCTYPE html>
    ...
    <body>
         ...
         if ($_SESSION['user_type'] == 'admin'||$_SESSION['user_type'] == 'Super admin'){
            ...
            //Inserting data
               ...
               $sql="INSERT INTO malescores(playerID,setswon,status,matchID,stageID,tournamentID,categoryId,points) VALUES('$playerId','$setswon','$status','$matchId','$stageId','$tournamentId','$categoryId','$points')";
               if($conn->query($sql)) {
                       ?>
                        <center>
  ...
  <br><br>   
		...
		<form action="rsaddmalescores.php" method="POST">
			<?php    
        if ($_SESSION['user_type'] == 'admin'||$_SESSION['user_type'] == 'Super admin'){
			?>
			     <div class="card card-container">
              <p id="profile-name" class="profile-name-card" style="font-size:18px;">Add male player scores
              </p>
...
<input class="btn btn-lg btn-primary btn-block btn-signin" type="submit" name="add_male_score" style="width:269px; height:40px;color:white;font-size:16px" value="Add scores">
