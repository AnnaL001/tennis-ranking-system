...
<!DOCTYPE html>
    ...
    <body>
         ...
         if ($_SESSION['user_type'] == 'admin'||$_SESSION['user_type'] == 'Super admin'){
            ...
            //Inserting data
               ...
               $sql="INSERT INTO matches(hostId,guestId,stageId,matchdate) VALUES('$hostId','$guestId','$stageId','$matchdate')";
               if($conn->query($sql)) {
                       ?>
                        <center>
  ...
  <br><br>   
		...
		<form action="rsaddmatches.php" method="POST">
			<?php    
        if ($_SESSION['user_type'] == 'admin'||$_SESSION['user_type'] == 'Super admin'){
			?>
			     <div class="card card-container">
              <p id="profile-name" class="profile-name-card" style="font-size:18px;">Add match
              </p>
...
<input type="date" name="matchdate" id="inputEmail" class="form-control" placeholder="Match date(YYYY-MM-DD)" required><br>
  ...
  <input class="btn btn-lg btn-primary btn-block btn-signin" type="submit" name="add_match" style="width:269px; height:40px;color:white;font-size:16px" value="Add match">
